...
async function resolverPlacaFotoOperativa(fotoUrl, telefono) {
  ...
  if (lectura.valida && placaDetectada) {
    var cargaExacta = await vehiculosData.cargarVehiculoYConductor(placaDetectada, telefono);
  ...
  if (placaDetectada) {
    placaSugerida = await vehiculosData.buscarPlacaSugerida(placaDetectada);
  }

  if (placaSugerida) {
    var cargaSugerida = await vehiculosData.cargarVehiculoYConductor(placaSugerida, telefono);
